...
public class ThreadSafeMapSynchronized<K, V> implements MyThreadSafeMap<K, V> {
    private volatile Set<MyEntry<K, V>> data;
    private final Object mySynchronization;

    public ThreadSafeMapSynchronized() {
        data = new HashSet<>();
        mySynchronization = this;
    }

    public ThreadSafeMapSynchronized(Collection<MyEntry<K, V>> data) {
        if (isNull(data)) {
